...
function renderTable() {
  ...
  appsTbody.innerHTML = appsCache.map((app) => `
    <tr>
      <td>${app.icon_path ? `<img class="thumb" src="/uploads/icons/${app.icon_path}" alt="" />` : ''}</td>
      <td>${app.name}</td>
      ...
      <td style="font-family:var(--font-mono)">${app.download_count.toLocaleString('en-US')}</td>
      <td>
        <div class="row-actions">
          <button class="icon-btn" data-edit="${app.id}">تعديل</button>
          <button class="icon-btn icon-btn--danger" data-delete="${app.id}">حذف</button>
        </div>
      </td>
    </tr>
  `).join('');
  ...
  appsTbody.querySelectorAll('[data-edit]').forEach((btn) =>
    btn.addEventListener('click', () => openForm(appsCache.find((a) => a.id === btn.dataset.edit)))
  );
